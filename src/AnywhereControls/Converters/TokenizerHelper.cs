    ...
    internal class TokenizerHelper
    {
        ...
        internal void LastTokenRequired()
        {
            if (_charIndex != _strLen)
                throw new InvalidOperationException($"Premature string termination encountered while parsing '{_str}'.");
        }
        ...
        internal bool NextToken(bool allowQuotedToken, char separator)
        {
            ...
            if (quoteCount > 0)
                throw new InvalidOperationException($"Missing end quote encountered while parsing '{_str}'.");
            ...
            if (_currentTokenLength < 1)
                throw new InvalidOperationException($"Empty token encountered at position {_charIndex} while parsing '{_str}'.");
        ...
        void ScanToNextToken(char separator)
        {
            ...
            if (_charIndex < _strLen)
            {
                ...
                if (!(currentChar == separator) && !char.IsWhiteSpace(currentChar))
                    throw new InvalidOperationException($"Extra data encountered at position {_charIndex} while parsing '{_str}'.");
                ...
                while (_charIndex < _strLen)
                {
                    ...
                    if (currentChar == separator)
                    {
                        ...
                        if (argSepCount > 1)
                            throw new System.InvalidOperationException($"Empty token encountered at position {_charIndex} while parsing '{_str}'.");
                    }
